document.addEventListener('DOMContentLoaded', function() {
    ...
    for (let i = 1; i <= topCards.length; i++) {
        ...
        if (topCard && bottomCard && dropTrigger) {
            ...
            Line below not optimal for zoom/window-resizing, use "style.transform" and "translate" instead of "style.top":
            
            cardGroups[i-1].bottomCard.style.top = `${cardGroups[i-1].getTopCardContainerPosition().bottom - cardGroups[i-1].bottomCard.getBoundingClientRect().height}px`;
            //-------------------------------------------------------
            //"translate" example
            let a = cardGroups[i-1].getTopCardContainerPosition().bottom; 
            let b = cardGroups[i-1].bottomCard.getBoundingClientRect().height;
            a -= b;console.log(a);
